...
class SearchAdapter(val context: Context, datas: MutableList<Datas>)
    ...
    override fun convert(helper: BaseViewHolder, item: Datas?) {
        ...
        helper.setText(R.id.homeItemTitle, Html.fromHtml(item.title))
                ...
                .setText(R.id.homeItemDate, if (item.originId != 0) context.getString(R.string.bookmark_time, item.niceDate) else item.niceDate)
                ...
                .setImageResource(R.id.homeItemLike, if (item.originId != 0 || item.collect) R.drawable.ic_action_like else R.drawable.ic_action_no_like)
